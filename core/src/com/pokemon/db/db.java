...
public class db {
    ...
    public static String sP(String userID, int num) {
        String sql = "Select pm_name from pm_info where pm_id = (Select pm_id from pm where u_id = '" + userID + "' and PM_BATTLE = '" + num + "');";
        String name = null;
        try {
            stmt = con.createStatement();
            rs = stmt.executeQuery(sql);
            while (rs.next()) {
                name = rs.getString("pm_name");
            }
            return name;
        } catch (SQLException e) {
            System.out.println("로그인SQL문이 틀렸습니다.");
