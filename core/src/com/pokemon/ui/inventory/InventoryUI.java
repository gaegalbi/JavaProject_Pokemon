...
import com.pokemon.ui.inventory.MovingImageUI;
...
public class InventoryUI extends AbstractUi {
    ...
    public InventoryUI(final GameScreen gameScreen,final Pokemon game, Player player) {
        ...
        assetManager.load("inven/inven.jpg", Texture.class);
        ...
        skin_o = SkinGenerator.generateSkin_O(assetManager);
        ...
        Label.LabelStyle[] labelColors = new Label.LabelStyle[] {
                new Label.LabelStyle(skin.getFont("font"), new Color(1, 1, 1, 1)), // white
                new Label.LabelStyle(skin.getFont("font"), new Color(0, 190 / 255.f, 1, 1)), // blue
                ...
                new Label.LabelStyle(skin.getFont("font"), new Color(220 / 255.f, 0, 0, 1)) // red
        };
        ...
        invButtonLabels[0].setFontScale(0.5f);
      ...
      /*  // enchanting
        invButtons[0].addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                if (currentItem != null && currentItem.getType() >= 2 && currentItem.getType() <= 6) {
                    //new Dialog("Enchant", rm.dialogSkin) {
                    new Dialog("Enchant", skin) {
                        {
                            Label l = new Label("Enchant " + currentItem.labelName + "\nfor " + currentItem.name + " g?",skin);
                            l.setFontScale(0.5f);
        ...
        if (toggle) {
            if (!currentItem.getEquipped()) {
                for (int i = 0; i < 2; i++) {
                    ...
                    if (currentItem.getType() < 2 || currentItem.getType() == 10) {
                        invButtons[0].setTouchable(Touchable.disabled);
